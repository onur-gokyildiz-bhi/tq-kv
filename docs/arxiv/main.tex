...
All 7 TurboQuant impl. & Various & 4-bit KV & FP16 only tested \\
\bottomrule
\end{tabular}
\end{table}
...
Three independent groups---ikawrakow (ik\_llama.cpp), spiritbuun (CUDA), and scos-lab---concluded that QJL hurts quality~\citep{llamacpp_tq}. Their reasoning: QJL variance is amplified by softmax, causing wrong token rankings. Response: disable QJL entirely.

\subsection{Our Counterargument}

These findings used \emph{dense} QJL with high-variance random projection. SRHT QJL has lower variance. We measure \textbf{attention KL divergence} (not just MSE):
...
SRHT QJL consistently reduces KL divergence by $\sim$2.9$\times$ at all context lengths. Top-5 token accuracy improves from 80--85\% to 87--95\%.

\subsection{Adaptive Activation}

We introduce \texttt{QjlMode::Adaptive\{threshold\}}: QJL off below threshold (minimal variance), on above (bias correction). Default threshold: 4096 tokens.
